...
--- Draw a glyph centered on the last item using plain text (no ImDrawList).
--- Fallback for when ImDrawListAddText is unavailable.
--- Draws the glyph as regular ImGui text centered in the available width.
---@param iconName string IconGlyphs name
function M.CenteredText(iconName)
    local glyph = M.Get(iconName)
    if not glyph then return end

    local availableWidth = ImGui.GetContentRegionAvail()
    local textWidth = ImGui.CalcTextSize(glyph)
...
-- ============================================================================
-- Index helpers (for icon browser, category lookups)
-- ============================================================================

--- Get a sorted list of all IconGlyphs names
...
---@return number
function M.Count()
    if not IconGlyphs then return 0 end
    local count = 0
    for _, glyph in pairs(IconGlyphs) do
        if type(glyph) == "string" and glyph ~= "" then
            count = count + 1
        end
    end
