...
---@field init fun(core: any, colorEngine: any, themes: any): nil
...
---@type table<string, table<string, ColorRGBA>> Color cache keyed by composite key → role → color
local _colorCache = {}
---@type string|nil Current cache key
local _cacheKey = nil
...
---@param core any Core module reference
---@param colorEngine any ColorEngine module reference
---@param themes any Theme definitions module reference
...
---@return ColorRGBA Color {r, g, b, a}
function M.color4n(role)
    ...
    if log then log.trace("Cache miss for role '" .. role .. "' (key: " .. key .. ")") end
...
function M.invalidateCache()
    _colorCache = {}
    _cacheKey = nil
    if log then log.debug("Token color cache invalidated") end
end
